 ...
 * @author Casper Nilsen - caspern@stud.ntnu.no
 */
...
/**
 * A ImageMapClientNav uses an OccupancyGridClient to create a map for use with a Navigator.
 *
 * @constructor
...
NAV2D.Navigator = function(options) {
  ...
  function sendGoal(pose) {
    ...
    path.subscribe(function(message) {
      ...
      that.rootObject.addChild(targetPath);
    });
...
/**
  ...
  if (withOrientation === false) {
    ...
    var yDelta = 0;
    var mapClick = false;
    var canvas = this.rootObject.canvas;
    var mouseEventHandler = function(event, mouseState) {
      if (mouseState === "down") {
        ...
        var img_rgba = ctx.getImageData(x,y,1,1).data;

        // if clicked pixel in canvas is white
        if (img_rgba[0] == 255 && img_rgba[1] == 255 && img_rgba[2] == 255) {
          mapClick = true;
        } else {
          mapClick = false;
        }
